...
MiningBeamFire = WeaponAbility:new()

function MiningBeamFire:init()
...
function MiningBeamFire:update(dt, fireMode, shiftHeld)
...
function MiningBeamFire:fire()
  ...
  while self.fireMode == (self.activatingFireMode or self.abilitySlot) and status.overConsumeResource("energy", (self.energyUsage or 0) * self.dt) do
    ...
    if collidePoint then
      ...
      world.damageTileArea(
        beamEnd,
        self.miningRadius,
        "foreground",
        beamEnd,
        "explosive",
        self.miningDamageAmount,
        self.harvestLevel
      )
...
function MiningBeamFire:drawBeam(endPos, didCollide)
...
function MiningBeamFire:cooldown()
...
function MiningBeamFire:firePosition()
  return vec2.add(mcontroller.position(), activeItem.handPosition(self.weapon.muzzleOffset))
end

function MiningBeamFire:aimVector(inaccuracy)
  local aimVector = vec2.rotate({1, 0}, self.weapon.aimAngle + sb.nrand(inaccuracy, 0))
  aimVector[1] = aimVector[1] * mcontroller.facingDirection()
  return aimVector
end

function MiningBeamFire:uninit()
  self:reset()
end

function MiningBeamFire:reset()
  self.weapon:setDamage()
  activeItem.setScriptedAnimationParameter("chains", {})
  animator.setParticleEmitterActive("beamCollision", false)
