...
function SoundBurst:fire()
  ...
  while self.fireMode == (self.activatingFireMode or self.abilitySlot) and status.overConsumeResource("energy", (self.energyUsage or 0) * self.dt) and self.playTimer > 0 do
    self.playTimer = math.max(0, self.playTimer - self.dt)
  ...
  self.cooldownTimer = self.fireTime - self.stances.fire.cooldown
