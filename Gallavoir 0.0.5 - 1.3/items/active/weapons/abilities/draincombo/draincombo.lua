...
function MeleeCombo:init()
  ...
  self.weapon:addTransformationGroup("swoosh", self.swooshOffset or {0,0}, 0)
...
function MeleeCombo:fire()
  ...
  util.wait(stance.duration, function()
    ...
    local nearEntities = self:findTargets(self.scanPoint or {1,0}, 1.5)
    
    for _, target in pairs(nearEntities) do
...
function MeleeCombo:findTargets(pos, range)
...
function MeleeCombo:debugQuery(pos, range)
  world.debugPoly({
...
function MeleeCombo:rotateWorldPos(vector)
