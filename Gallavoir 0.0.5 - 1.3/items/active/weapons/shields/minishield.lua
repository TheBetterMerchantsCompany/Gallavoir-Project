...
function init()
  ...
  self.shieldEnergyRegen = config.getParameter("shieldEnergyRegen", 0.1)
  
  self.effectImmunity = config.getParameter("effectImmunity", false)
  
  self.dodec = config.getParameter("dodec", true)
...
function update(dt, fireMode, shiftHeld)
  ...
  if self.active then
    ...
    self.dodec.rot[1] = self.aimAngle * self.aimDirection
    
    if self.aimDirection == -1 then self.dodec.rot[1] = self.dodec.rot[1] + math.pi end
    
    activeItem.setScriptedAnimationParameter("dodec", self.dodec)

    if status.resourcePositive("perfectBlock") then
      animator.setGlobalTag("directives", self.perfectBlockDirectives)
      animator.setAnimationState("shield", "raisedPerfect")
    else
      animator.setGlobalTag("directives", "")
      animator.setAnimationState("shield", "raised")
    end
    
    --sb.logInfo("%s",status.stat("shieldHealth"))
    --sb.logInfo("%s",status.resource("shieldStamina"))
    
    if (status.resource("shieldStamina") < 1.0) then
      if status.overConsumeResource("energy", (self.energyUsage or 0) * dt) then
       status.giveResource("shieldStamina", (dt * self.shieldEnergyRegen))
     end
    end
...
function updateAim()
  ...
  if self.stance.allowRotate then
    ...
    self.aimAngle = self.stance.aimAngle or self.aimAngle
  end
...
function setStance(stance)
  ...
  activeItem.setTwoHandedGrip(stance.twoHanded or self.twoHanded or false)
end
...
function raiseShield()
  ...
  if self.effectImmunity then
    status.setPersistentEffects("ShieldEffectImmunity",
...
function lowerShield()
  setStance(self.stances.idle)
  animator.setGlobalTag("directives", "")
  activeItem.setScriptedAnimationParameter("dodec", false)
  animator.setAnimationState("shield", "idle")
  animator.playSound("lowerShield")
  self.active = false
  self.activeTimer = 0
  status.clearPersistentEffects(activeItem.hand().."Shield")
  status.clearPersistentEffects("ShieldEffectImmunity")
  activeItem.setItemShieldPolys({})
  activeItem.setItemDamageSources({})
